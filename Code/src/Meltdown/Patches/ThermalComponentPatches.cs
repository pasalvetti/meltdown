...
namespace Meltdown.Patches
{
    internal class ThermalComponentPatches
    {
        ...
        /**
         * The heat removing code is originally done by ThermalComponentJob.FinalizeJob but the way it is not well balanced right now with the rest of the system.
        ...
        [HarmonyPatch(typeof(ThermalComponent), nameof(ThermalComponent.OnUpdate))]
        [HarmonyPostfix]
        public static void OnUpdatePostFix(ThermalComponent __instance)
        {
            foreach (PartComponent part in __instance.SimulationObject.PartOwner.Parts)
            {
                //part.ThermalData.CoolingEnergyToApply = -10000000.0;
            }
        }
    }
}
